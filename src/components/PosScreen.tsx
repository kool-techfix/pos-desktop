...
import type { CartItem, Product, Sale } from "@/types/types";

import "./PosScreen.scss";
import { useApp } from "@/providers/AppProvider";

export function PosScreen() {
  const { state, user, createSale, signOut } = useApp();
  ...
  const finishSale = () => {
    if (!cart.length || paid < total || !user) {
      return;
    }
      ...
      <div className="pos-page__layout">
        ...
        <section className="panel pos-page__basket">
          ...
          <div className="pos-page__payment">
            ...
            <div
              className={`pos-page__change ${
                change >= 0 && total > 0 ? "pos-page__change--ready" : ""
              }`}
            >
              <span>
            ...
            <button
              ...
              onClick={finishSale}
              disabled={!cart.length || paid < total || !user}
